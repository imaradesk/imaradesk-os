...
export default function CustomerPortalTicketDetail({ ticket, comments, activity_logs, currentUser, tenant_name }) {
  ...
  const handleAddComment = async (e) => {
    ...
    try {
      const response = await api.post(`/portal/ticket/${ticket.ticket_number}/add-comment/`, {
        message: newComment,
      });
  ...
  const formatShortDate = (dateString) => formatShortDateTime(dateString);
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gray-50">
        {/* Header */}
        <header className="shadow-sm" style={{ background: COLORS.primary }}>
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className={`px-3 py-1 rounded-full text-xs font-medium ${STATUS_COLORS[ticket.status] || 'bg-gray-100 text-gray-800'}`}>
                      {ticket.status.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase())}
                    </span>
                  </div>
                  <p className="text-sm text-gray-600">
                    Ticket #{ticket.ticket_number}
                    {ticket.assignee && <span className="ml-3">Assigned to: <span className="font-medium">{ticket.assignee}</span></span>}
                  </p>
                </div>
                <span className={`px-4 py-2 rounded-lg text-sm font-medium ${PRIORITY_COLORS[ticket.priority] || 'bg-gray-100 text-gray-800'}`}>
            ...
            <div className="border-b border-gray-200">
              <nav className="flex -mb-px">
                <button
                  ...
                  style={activeTab === 'comments' ? { borderColor: COLORS.primary, color: COLORS.primary } : {}}
                >
                ...
                <button
                  ...
                  style={activeTab === 'activity' ? { borderColor: COLORS.primary, color: COLORS.primary } : {}}
                >
                  ...
                  {localComments.length > 0 ? (
                    <div className="space-y-6 mb-8">
                      {localComments.map((comment) => (
                        <div
                          key={comment.id}
                          className={`relative pl-6 pb-6 border-l-2 ${comment.is_agent ? '' : 'border-gray-300'}`}
                          style={comment.is_agent ? { borderColor: COLORS.primary } : {}}
                        >
                          <div
                            className="absolute left-[-9px] top-0 w-4 h-4 rounded-full bg-white border-2"
                            style={comment.is_agent ? { borderColor: COLORS.primary } : { borderColor: '#d1d5db' }}
                          />
                          <div className="bg-white rounded-lg">
                            <div className="flex items-center justify-between mb-3">
                              <div className="flex items-center gap-2">
                                <span className="font-medium text-sm" style={comment.is_agent ? { color: COLORS.primary } : {}}>
                                  {comment.author}
                                </span>
                                {comment.is_agent ? (
                    ...
                    <div className="space-y-4">
                      {activity_logs.map((activity) => (
                        <div key={activity.id} className="flex items-start gap-4 pb-4 border-b border-gray-100 last:border-0">
                          <div className="flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center" style={{ backgroundColor: `${COLORS.primary}20` }}>
